...
#' 나라 전체 종목 VPIN 병렬 계산
run_vpin_all <- function(vpin_ticks, country, log_file = NULL) {
  output_dir     <- get_output_dir(country, "vpin")
  checkpoint_dir <- get_checkpoint_dir(country, "vpin")
  all_symbols    <- make_symbol_queue(vpin_ticks)
  ...
  if (length(remaining) == 0) {
    ...
    for (bi in seq_along(batch_starts)) {
      ...
      batch_results <- furrr::future_map(batch_syms, function(sym) {
        ...
        sym_data <- vpin_ticks[vpin_ticks$Symbol == sym, ]
        tryCatch(
          run_vpin_worker(sym_data, sym, checkpoint_dir, log_file = log_file),
          error = function(e) {
            empty <- data.frame(
              symbol = character(0), bucket = integer(0),
              bucket_date = as.Date(character(0)),
