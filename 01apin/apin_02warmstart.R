...
#     · 이후 윈도우 (직전 추정 성공):
#         warm-start 1행 + GE 초기점 NUM_INIT_ROLL행을 rbind하여 전달.
#         PINstimation의 initialsets가 dataframe일 때 num_init이 무시되므로,
#         initials_adjpin()으로 GE 초기점을 별도 생성 후 병합하는 방식으로 우회.
#         이를 통해 warm-start의 빠른 수렴 + GE의 다양한 탐색을 동시에 확보.
#     · 이후 윈도우 (직전 추정 실패):
...
# 적응형 num_init 설정
NUM_INIT_FIRST <- 20   # 첫 번째 윈도우 또는 직전 실패 시 (GE 초기점 개수)
NUM_INIT_ROLL  <-  5   # 직전 성공 후 warm-start와 병합할 GE 초기점 개수
...
cat(sprintf("  num_init    : 첫 윈도우 = %d  |  롤링 = 1(warm) + %d(GE)\n",
...
worker_process_symbol <- function(args) {
  ...
  if (n_days >= window_size) {
    for (i in window_size:n_days) {
      ...
      # 적응형 초기화: warm-start 1행 + GE 초기점을 rbind하여 전달
      # PINstimation에서 initialsets가 dataframe이면 num_init이 무시되므로,
      # initials_adjpin()으로 GE 초기점을 별도 생성 후 warm-start와 병합.
      # → warm-start의 빠른 수렴 + GE의 다양한 봉우리 탐색을 동시에 확보.
      if (!is.null(prev_params)) {
        warm_df <- make_warm_start_df(prev_params)
        ge_df   <- tryCatch(
          initials_adjpin(window_df, xtraclusters = 4, verbose = FALSE),
          error = function(e) NULL
        )
        if (!is.null(ge_df) && nrow(ge_df) > 0) {
          ge_df <- head(ge_df, num_init_roll)
          cur_initialsets <- rbind(warm_df, ge_df)
        } else {
          cur_initialsets <- warm_df
        }
        cur_num_init <- nrow(cur_initialsets)
      } else {
